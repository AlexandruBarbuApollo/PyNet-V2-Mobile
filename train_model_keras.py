...
from load_dataset import load_train_patch, load_val_data, load_real_test_data
...
# if gpus:
#     gpu = gpus[0]  # Assuming you have only one GPU, change index if needed
#     try:
...
with tf.compat.v1.Session() as sess:
    ...
    class TestEvaluationCallback(Callback):
        def __init__(self, test_data, test_answ):
            super().__init__()
            self.test_data = test_data
            self.test_answ = test_answ

        def on_epoch_end(self, epoch, logs=None):
            evaluation = self.model.evaluate(self.test_data, self.test_answ, batch_size=test_batch_size)
            test_loss = evaluation[0]
            test_psnr = evaluation[1]
            test_ssim = evaluation[2]
            print(f"\nTest Evaluation at end of epoch {epoch + 1}:")
    ...
    history = model.fit(
        ...
        workers=1, use_multiprocessing=False, callbacks=[reduce_lr, save_model, save_best_model, csv_logger, early_stopping, test_eval]
    )
