...
def extract_bayer_channels(raw):
    ...
    RAW_norm = RAW_combined.astype(np.float16) / (4 * 255)
...
def load_val_data(dataset_dir, dslr_dir, phone_dir, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE):
    ...
    val_directory_dslr = dataset_dir + 'valid/' + dslr_dir
    val_directory_phone = dataset_dir + 'valid/' + phone_dir
    ...
    val_data = np.zeros((NUM_VAL_IMAGES, PATCH_WIDTH, PATCH_HEIGHT, 4), dtype=np.float16)
    val_answ = np.zeros((NUM_VAL_IMAGES, int(PATCH_WIDTH * DSLR_SCALE), int(PATCH_HEIGHT * DSLR_SCALE), 3), dtype=np.float16)

    VAL_IMAGES = [file_name.replace('.png', '') for file_name in os.listdir(val_directory_phone)]

    #for i in range(0, NUM_VAL_IMAGES):
    for i, name in enumerate(VAL_IMAGES):

        I = np.asarray(imageio.imread((val_directory_phone + str(name) + '.png')))
        I = extract_bayer_channels(I)
        val_data[i, :] = I

        I = Image.open(val_directory_dslr + str(name) + '.' + format_dslr)
...
def load_real_test_data(dataset_dir, dslr_dir, phone_dir, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE):
...
def load_train_data(dataset_dir, dslr_dir, phone_dir, TRAIN_SIZE, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE):
    ...
    train_data = np.zeros((NUM_TRAINING_IMAGES, PATCH_WIDTH, PATCH_HEIGHT, 4), dtype=np.float16)
    train_answ = np.zeros((NUM_TRAINING_IMAGES, int(PATCH_WIDTH * DSLR_SCALE), int(PATCH_HEIGHT * DSLR_SCALE), 3), dtype=np.float16)

    i = 0
    TRAIN_IMAGES = [file_name.replace('.png', '') for file_name in os.listdir(train_directory_phone)]
    TRAIN_IMAGES = np.random.choice(TRAIN_IMAGES, TRAIN_SIZE, replace=False)
    #for i in range(0, NUM_VAL_IMAGES):
...
def load_train_patch(dataset_dir, dslr_dir, phone_dir, TRAIN_SIZE, PATCH_WIDTH, PATCH_HEIGHT, DSLR_SCALE):
    ...
    train_data = np.zeros((TRAIN_SIZE, PATCH_WIDTH, PATCH_HEIGHT, 4), dtype=np.float16)
    train_answ = np.zeros((TRAIN_SIZE, int(PATCH_WIDTH * DSLR_SCALE), int(PATCH_HEIGHT * DSLR_SCALE), 3), dtype=np.float16)
